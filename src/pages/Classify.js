...
export default class Classify extends Component {
  ...
  initWebcam = async () => {
    try {
      ...
      console.error("Webcam initialization error:", e); // Log the error to the console
      this.refs.noWebcam.style.display = 'block';
      // this.refs.noWebcam.textContent = `Error: ${e.message}`; // Display the error message on the page
    }
  }
  ...
  classifyWebcamImage = async () => {
    ...
    // Check that imageData has the correct shape
    console.log('Image Data shape:', imageData.shape);
    const logits = this.model.predict(imageData);
    console.log('Logits shape:', logits.shape); // Check logits shape
    console.log('Logits:', logits); // Debugging line
    const probabilities = await logits.data();
    console.log('Probabilities:', probabilities); // Debugging line
    console.log('Length of probabilities:', probabilities.shape); // Check the length of the probabilities
  ...
  getTopKClasses = async (values, topK) => {
    ...
    //   console.warn('The values array is empty or not an array.');
    //   return []; // Return an empty array if values is invalid
    //   }

    // Check if the output is a single value and handle it
    if (values.length === 1) {
      let classof = 0
      if (values[0]>= 0.5){
        classof = 0
      ...
      return [{
          className: MODEL_CLASSES[classof], // Assuming there's at least one class
          probability: (values[0] * 100).toFixed(2)
      }];
  }
    ...
    for (let i = 0; i < topkIndices.length; i++) {
      ...
      if (classIndex >= 0 && classIndex < MODEL_CLASSES.length) {
        topClassesAndProbs.push({
          className: MODEL_CLASSES[classIndex],
          probability: (topkValues[i] * 100).toFixed(2)
        });
      } else {
        console.warn(`Index ${classIndex} is out of bounds for MODEL_CLASSES`);
      }
    }
          ...
          <div className="classification-results">
            ...
            <ListGroup>
              {this.state.predictions.map((category) => {
                const isFoodWaste = category.className.toLowerCase() === "food waste";
                const label = isFoodWaste ? "FOOD WASTE" : "OTHERS";
                const color = isFoodWaste ? "green" : "red";
